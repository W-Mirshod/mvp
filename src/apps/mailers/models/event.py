...
class Event(DeleteModelMixin, DateModelMixin, models.Model):
    ...
    def create_new_event(cls, user, server, template="", results=None):
        if results is None:
            results = {}
        sent_message = SentMessage.objects.create(user=user, template=template, results=results)
        event = cls.objects.create(server=server, status=StatusType.NEW, sent_message=sent_message)
        return event
