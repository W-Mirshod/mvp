...
class IMAPHealthCheck(BaseHealthCheckBackend):

    critical_service = False

    def check_status(self) -> bool:
        try:
            url = MAIN_HOST + reverse_lazy("servers_api:imap-server_list")
            ...
            SendToSentry.send_scope_msg(
                scope_data={
                    "message": f"IMAPHealthCheck.check_status: Exception",
                    "level": SentryConstants.SENTRY_MSG_ERROR,
                    "tag": SentryConstants.SENTRY_TAG_REQUEST,
                    "detail": f"IMAP API health check failed",
                    "extra_detail": str(ex),
                }
            )
