...
class ProductHealthCheck(BaseHealthCheckBackend):

    critical_service = False

    def check_status(self) -> bool:
        try:
            url = MAIN_HOST + reverse_lazy("products_api:product_list")
            ...
            SendToSentry.send_scope_msg(
                scope_data={
                    "message": f"ProductHealthCheck.check_status: Exception",
                    "level": SentryConstants.SENTRY_MSG_ERROR,
                    "tag": SentryConstants.SENTRY_TAG_REQUEST,
                    "detail": f"products api health check failed",
                    "extra_detail": str(ex),
                }
            )
