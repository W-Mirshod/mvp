...
class CompaniesHealthCheck(BaseHealthCheckBackend):

    critical_service = False

    def check_status(self) -> bool:
        try:
            url = MAIN_HOST + reverse_lazy("companies_api:company_list")
            ...
            SendToSentry.send_scope_msg(
                scope_data={
                    "message": f"CompaniesHealthCheck.check_status: Exception",
                    "level": SentryConstants.SENTRY_MSG_ERROR,
                    "tag": SentryConstants.SENTRY_TAG_REQUEST,
                    "detail": f"Companies app health check failed",
                    "extra_detail": str(ex),
                }
            )
